...
void print_option_name(bool is_stdout, std::string_view name, std::string_view arg_placeholder = "") {
    if (is_stdout) {
...
void print_help() {
    ...
    fmt::print("    This section provides a detailed description of your program.\n\n");
    ...
    print_option_name(false, "A file or directory to search. Directories are searched recursively.");
    print_option_name(false, "If a git repository is detected, its git index is loaded and the");
    print_option_name(false, "index entries are iterated using libgit2 and searched. If a path is not");
    print_option_name(false, "provided, the current working directory is recursively searched.\n");
    ...
    print_description_line("Print the 0-based byte offset within the input file before each");
    print_description_line("line of output.If -o (--only-matching) is used, print the offset");
    print_description_line("of the matching part itself.\n");

    // Column
    print_option_name(is_stdout, "--column");
    print_description_line("Show column numbers (1-based). This only shows the column");
    print_description_line("numbers for the first match on each line.\n");

    // Count
    print_option_name(is_stdout, "-c, --count");
    print_description_line("This flag suppresses normal output and shows the number of");
    print_description_line("lines that match the given pattern for each file searched\n");

    // Count Matches
    print_option_name(is_stdout, "--count-matches");
    print_description_line("This flag suppresses normal output and shows the number of");
    print_description_line("individual matches of the given pattern for each file searched\n");

    // Exclude git submodules
    print_option_name(is_stdout, "--exclude-submodules");
    print_description_line("For any detected git repository, this option will cause");
    print_description_line("hypergrep to exclude any submodules found.\n");

    // Files
    print_option_name(is_stdout, "--files");
    print_description_line("Print each file that would be searched without actually");
    ...
    print_option_name(is_stdout, "        hg --filter '(include|src)/.*\\.(c|cpp|h|hpp)'\n");
    print_description_line("will search C/C++ files in the any */include/* and */src/* paths.\n");

    // Fixed Strings
    print_option_name(is_stdout, "-F, --fixed-strings");
    print_description_line("Treat the pattern as a literal string instead of a regex.");
    print_description_line("Special regex meta characters such as .(){}*+ do not need");
    ...
    print_description_line("Search hidden files and directories. By default, hidden files");
    print_description_line("and directories are skipped. A file or directory is considered");
    print_description_line("hidden if its base name starts with a dot character ('.').\n");

    // Ignore case
    print_option_name(is_stdout, "-i, --ignore-case");
    print_description_line("When this flag is provided, the given patterns will be searched");
    ...
    print_description_line("Print the paths with at least one match and suppress match contents.\n");

    // Max columns
    print_option_name(is_stdout, "-M, --max-columns", "<NUM>");
    print_description_line("Don't print lines longer than this limit in bytes. Longer lines are");
    print_description_line("omitted, and only the number of matches in that line is printed.\n");

    // Max filesize
    print_option_name(is_stdout, "--max-filesize", "<NUM+SUFFIX?>");
    print_description_line("Ignore files above a certain size. The input accepts suffixes of");
    print_description_line("form K, M or G. If no suffix is provided the input is treated as bytes");
    ...
    print_description_line("Show line numbers (1-based). This is enabled by defauled when searching");
    print_description_line("in a terminal.\n");

    // No Line Number
    print_option_name(is_stdout, "-N, --no-line-number");
    print_description_line("Suppress line numbers. This is enabled by default when not searching in");
    print_description_line("a terminal.\n");

    // Only matching parts
    print_option_name(is_stdout, "-o, --only-matching");
    print_description_line("Print only matched parts of a matching line, with each such part on a");
    print_description_line("separate output line.\n");

    // Binary as text
    print_option_name(is_stdout, "-a, --text");
    print_description_line("Search binary files as if they were text. When this flag is present,");
    print_description_line("binary file detection is disabled. When a binray file is searched, its");
    print_description_line("contents may be printed if there is a match. This may cause escape");
    print_description_line("codes to be printed that alter the behavior of the terminal.\n");

    // UCP
    print_option_name(is_stdout, "--ucp");
    print_description_line("Use unicode properties, rather than the default ASCII interpretations,");
    print_description_line("for character mnemonics like \\w and \\s as well as the POSIX");
